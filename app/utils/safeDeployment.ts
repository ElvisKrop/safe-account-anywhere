...
export async function verifySafeDeployment(rpcUrl: string, safeAddress: string) {
  try {
    const client = createPublicClientForChain(rpcUrl)
...
export async function parseDeploymentTransaction(rpcUrl: string, txHash: string) {
  try {
    const client = createPublicClientForChain(rpcUrl)
...
export async function getFactoryAddress(sourceChainId: string, txHash: string) {
  try {
    ...
    const deploymentDetails = await parseDeploymentTransaction(chainInfo.rpcUrls[0], txHash)
